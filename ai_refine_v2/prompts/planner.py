...
on a polished light-gray studio floor with subtle silver-metallic gradient
backdrop (#F5F5F7 dominant). Product fills the center-right with its
EXACT yellow color preserved, no ambient color shifting. A bold white
display headline 'DZ600M 无人水面清洁机' anchors the upper-left with
generous negative space, a small condensed sans-serif subtitle 'Spiral
cleaning · 8h endurance' below in mid-gray (#86868B). Neutral cool studio
lighting from upper-left, soft fill, NO warm tints. Crisp clean
composition with quiet premium minimalist confidence — DJI/Apple-inspired."
...
✓ v3.2 合格示例 (大疆风高级灰 — 跟你产品颜色无关, 是格式示范, 不是色彩抄袭):
...
✓ v3.2 合格示例 (固定模板, 强烈推荐照抄, 之后只调 product 颜色描述):
...
product retains its EXACT original color (NOT recolored by ambient lighting);
...
product's original color is the ONLY saturated element on screen,
...
✓ 正例 v3.2 (商用清洁机器人 DZ70X 运作中):
"Asian male operator in plain navy work uniform supervising DZ70X scrubber
robot actively cleaning a polished marble shopping mall floor, water trail
visible behind robot, neutral cool studio lighting, light gray (#F5F5F7)
backdrop blends with mall environment, product retains its exact charcoal
black color (NOT recolored by ambient), headline 「智能作业 高效清洁」
upper-left in mid-gray sans-serif..."
...
[产品颜色保真硬约束 (v3.2 新增, deliberate_iron_rule_5_break_2nd 触发原因)]

Scott 实测 DZ70X (黑色商用清洁机器人) 用 v3.iter2 暖色路线 → 产品被染金色,
形态保真严重失败 (验收维度 2 FAIL). 公司产品矩阵颜色多样 (黄/黑/灰/银/白),
v3.2 必须保产品本色不被环境光污染.

产品颜色保真规则 (任一不符 = 验收维度 2 FAIL, 退回重写):
产品本身的颜色 (product_meta.primary_color 字段) 必须严格保留,
不允许被环境光 / 背景色 / 滤镜染色.

具体规则:
- 黄色产品 → 始终是黄色 (不变金色 / 橙色 / 暖黄)
- 黑色产品 → 始终是黑色 (不变暖灰 / 棕色 / 深褐)
- 灰色产品 → 始终是灰色 (不变暖灰 / 银色)
- 白色产品 → 始终是白色 (不变米色 / 暖白)
- 银色产品 → 始终是银色 (不变金色 / 香槟色)

prompt 表达 (强烈推荐照抄到每屏 prompt):
"Product retains its EXACT primary color (preserve from reference image),
NO ambient color shifting, NO warm tint contamination, NO recoloring by lighting."

INJECTION_PREFIX (generator 端实施, 准则 5.2 v3.2 强化版):
"Image 1 is the reference product cutout. Preserve the product's EXACT
original color WITHOUT ambient color shifting. The product's primary color
must remain unchanged regardless of lighting or background. Preserve
silhouette, key visual parts, and original hue exactly."
