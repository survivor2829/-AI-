...
- **[2026-04-21] 硬件升级暂缓决策 (OOM 事故修完当晚立)**: 2C2G 维持不升, 批量生成保持串行模式够用。
  **前提**:
  - 资金紧张, 批量生成属非紧急功能 (核心主业务 = 单品生成, 已在跑)
  - 现有架构: 旧单品生成路径正常 + 新批量生成走 `BATCH/SINGLE/REFINE_POOL_SIZE=1` 串行 → 慢但稳, OOM 风险已消
  - 阶段七"触发条件"定义不变 (见阶段七"性能扩展性分析"升级阈值表)
  **升级触发阈值 (任一命中立即启动 4C8G 升配, 走 "4C8G 升级具体步骤" 7 步)**:
  1. 真实付费客户投诉慢 ≥ 2 次
  2. 单批 ≥ 5 产品成为日常需求
  3. 日处理批次 ≥ 10 个
  未命中前: 2C2G 维持, 不主动升; 阶段八 UX 改进也暂缓 (4C8G 稳了之后再动, 详见阶段八"背景"段)。
