...
**物保云产品详情页自动生成器** —— 一个 Flask Web 应用，用于自动生成商用清洁机器人的产品详情页。
...
- **API Key 暴露**：`app.py:36` 中 DeepSeek API Key 硬编码在代码里（`sk-e1dd...`），**这是安全隐患**，应改为纯环境变量读取
...
- [ ] **DeepSeek API Key 泄露**：硬编码在 `app.py` 第 36 行，公开仓库可见
